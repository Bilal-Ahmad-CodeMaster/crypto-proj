    ...
    return (
        <div className="mb-3 rounded-lg border border-gray-200 bg-white hover:shadow-sm transition-all">
            <div className="flex flex-col md:flex-row">
                ...
                <div className={`w-full md:w-56 p-4 md:p-2 text-white relative ${positive ? 'bg-[#3FB488]' : 'bg-[#F15C5C]'}`}>
                    ...
                    <div className='flex flex-col w-full justify-end items-end mt-6 md:mt-0'>
                        <div className="text-xs opacity-90">Profit per {title}</div>
                        <div className="text-xl md:text-2xl font-bold mb-1">{money}</div>
                ...
                <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 items-center">
                    {/* Mined Column */}
                    <div className="text-center border-b sm:border-b-0 sm:border-r border-gray-200 py-4">
                        <div className="text-sm font-semibold text-gray-700 mb-2">Mined/{title.toLowerCase()}</div>
                        <div className={`text-lg font-bold ${positive ? 'text-green-700' : 'text-red-700'}`}>{mined}</div>
                    </div>

                    {/* Power Cost Column */}
                    <div className="text-center py-4">
                        <div className="text-sm font-semibold text-gray-700 mb-2">Power cost/{title.toLowerCase()}</div>
                        <div className={`text-lg font-bold ${positive ? 'text-green-700' : 'text-red-700'}`}>{powerCost}</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    return (
        <div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <div className={`rounded-xl p-4 md:p-6 shadow-md ${positive ? 'bg-[#C4E8DB]' : 'bg-[#FFE0E0]'}`}>
                    <div className="text-xs font-semibold bg-white/90 inline-block px-3 py-1 rounded-full shadow">PROFIT RATIO PER DAY</div>
                    <div className={`text-3xl md:text-4xl font-bold mt-4 ${positive ? 'text-green-700' : 'text-red-700'}`}>{positive ? '73%' : '-70%'}</div>
                </div>

                <div className={`rounded-xl p-4 md:p-6 shadow-md ${positive ? 'bg-[#C4E8DB]' : 'bg-[#FFE0E0]'}`}>
                    <div className="text-xs font-semibold bg-white/90 inline-block px-3 py-1 rounded-full shadow">PROFIT PER MONTH</div>
                    <div className={`text-3xl md:text-4xl font-bold mt-4 ${positive ? 'text-green-700' : 'text-red-700'}`}>{positive ? '$ 94.64' : '$ -90.66'}</div>
                </div>
            </div>
