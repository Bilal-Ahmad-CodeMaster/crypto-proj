...
function StatRow({ title, money, mined, powerCost, poolFee, positive = true }: StatRowProps) {
    return (
        <div className={`mb-3 rounded-lg border ${positive ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} hover:shadow-sm transition-all`}>
            <div className="p-4">
                {/* Label in corner */}
                <div className="flex justify-between items-start mb-4">
                    <div className={`text-xs font-semibold px-3 py-1 rounded-full ${positive ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'}`}>
                ...
                <div className="grid grid-cols-3 gap-4 items-center">
                    ...
                    <div className="text-center border-x border-gray-200 px-4">
                        <div className={`text-sm font-semibold mb-2 ${positive ? 'text-green-700' : 'text-red-700'}`}>Mined/{title.toLowerCase()}</div>
                        <div className={`text-lg font-bold ${positive ? 'text-green-800' : 'text-red-800'}`}>{mined}</div>
                    </div>

                    {/* Power Cost Column */}
                    <div className="text-center">
                        <div className={`text-sm font-semibold mb-2 ${positive ? 'text-green-700' : 'text-red-700'}`}>Power cost/{title.toLowerCase()}</div>
                        <div className={`text-lg font-bold ${positive ? 'text-green-800' : 'text-red-800'}`}>{powerCost}</div>
                    </div>
                </div>
            </div>
        </div>
